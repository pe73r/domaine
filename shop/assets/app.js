...
var header = {
  init: function(){
    ...
    $('[data-menu-toggle="parent-toggle"]').on('click', function(e) {
      var $toggle = $(this);
      if($toggle.next('[data-menu-toggle="visible-toggle"]').is(':visible')) {
        $toggle.next('[data-menu-toggle="visible-toggle"]').hide();
        $toggle.html('+');
        $toggle.attr('aria-expanded', false);
      } else {
        $toggle.next('[data-menu-toggle="visible-toggle"]').show();
        $toggle.html('-');
    ...
    $('.mobile-mega-menu .text_link').on('click', function(e) {
    ...
    /* Mega menu */
    $('body').on('mouseover', '.main .js-mega-menu', function() {
      $(this).prev('.sub-menu').attr('aria-expanded', true);
    });
    $('body').on('mouseout', '.main .js-mega-menu', function() {
      $(this).prev('.sub-menu').attr('aria-expanded', false);
      $(this).next('.js-mega-menu').removeClass(' animated fadeInUp');
    });
    $('body').on('mouseover', '.main .js-mega-menu-dropdown', function() {
      $(this).next('.js-mega-menu').addClass(' animated fadeInUp');
    });
    $('body').on('mouseout', '.main .js-mega-menu-dropdown', function() {
      $(this).next('.js-mega-menu').removeClass(' animated fadeInUp');
    });
    ...
    // Function for menu links dropping down on hover for desktop

    $('body').on('mouseover', '.main .sub-menu', function() {
      $(this).attr('aria-expanded', true);
    });
    $('body').on('mouseout', '.main .sub-menu', function() {
...
var slideshow = {
  init: function(){
    $('.homepage_slider').each(function (index, value){
      ...
      var slideAmount = homepageSlider.data('slide-amount');
      ...
      homepageSlider.flexslider({
        touch: true,
        controlNav: prevNextButtons,
        directionNav: prevNextButtons,
          ...
          $.ajax({
            ...
            success: function (data) {
              $nf.fadeOut("slow", function(){
                $nf.prev('.message').html("Merci ! Nous vous contacterons lorsque ce produit sera de nouveau disponible !");
              });
            },
            error: function(data) {
...
var map = {
  init: function(){
    if ($(".map-section .map, .contact-section .map, .page-details .map").hasClass('js-api-map')) {
      var mapsToLoad = [];
...
var faqAccordion = {
  init: function(){
    var flg = 0;
    var $faqHeading = $('.faq-accordion > dt > .faq-accordion--button');
      ...
      $(document).on('click', '.ajax-submit', function(e) {
        ...
        $.ajax({
          ...
          success: function(itemData) {
            $addToCartBtn.find('span').text("Ajouté").removeClass('zoomOut').addClass('fadeIn');
          ...
          error: function(XMLHttpRequest) {
            ...
            $addToCartBtn.removeAttr('disabled').removeClass('disabled');
            $addToCartBtn.find('span').text("Ajouter au panier").removeClass('zoomOut').addClass('zoomIn');
          }
        });
...
$(function() {
  ...
  if ($('.tos_agree').length) {
    $('body').on('click', '#checkout', function() {
      if ($('.tos_agree').is(':checked')) {
        ...
        alert("Vous devez accepter les termes et conditions à la caisse.");
        return false;
      }
    });
  }
  ...
  if ($('.shopify-section.page-details-section').length || $('body').hasClass('page-details')){
    // Don't apply mediaWrapper
  } else if ($('body').hasClass('index')) {
    $('.featured-product-section iframe[src*=youtube], .featured-product-section iframe[src*=vimeo], .twitter-feed-section iframe[src*=youtube], .twitter-feed-section iframe[src*=vimeo]').mediaWrapper({
      intrinsic: true
    });
  } else {
        ...
        $.ajax({
          ...
          success: function (data) {
            $cf.fadeOut("slow", function(){
              $cf.prev('.message').html("Merci de vous être inscrit(e) à notre liste de diffusion !");
            });
          },
          error: function(data) {
  ...
  selectCallback = function(variant, selector) {
    ...
    var notifyEmail = "Saisissez votre adresse e-mail...";
    var notifyEmailValue = "";
    var notifySend = "Envoyer";
        ...
        success: function(data) {
          ...
          if(data.meta.code === 200 && data.data.length) {
            ...
            for(var i = 0; i < data.length; i++) {
              ...
              if(item) {
                item = '<div class="instas instagram__item">'+item+'</div>';
              }
              if(item !== '') {
                s.el.append(item);
                count++;
...
$(document)
  .on('shopify:section:load', function(e){
    ...
    if ($parentSection.hasClass('map-section') || $parentSection.hasClass('contact-section') || $parentSection.hasClass('page-details-template')) {
      map.init();
    }
    ...
    if ($parentSection.hasClass('video-section') || $parentSection.hasClass('block__video') || $parentSection.hasClass('block__featured_video') || $('body').hasClass('page-details')){
      video.init();
    }
